...
The final runnable version is frozen in `replay/` because the shared scripts also serve a separate day-long study. It retains the five-hour defaults and its 58-category registry. Final pool-ordering features use the v4 manager address plus pool ID, and novelty features preserve every unknown topic. The Dolomite method also records whether resulting principal is positive, negative or zero.
