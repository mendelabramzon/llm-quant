...
classify  Offline, fast. Applies the rules of scripts/type_registry.py to every transaction above the threshold, runs
...
DAY = ROOT / 'research' / '2026-09-05' / 'eth_day'
OUT_DEFAULT = ROOT / 'research' / '2026-09-05' / 'amount_outliers_eth_day'
...
class Ctx:
    ...
    def __init__(self, r, profiles, thr):
        ...
        legs = r['legs']
        token_logs = set(r['fam']) <= {'ERC20_Transfer_shape', 'LidoTransferShares'}
        self.plain = ((int(r['v']) > 0 and r['lc'] == 0 and r['cd'] == 0) or
                      (r['sel'] == '0xa9059cbb' and r['lc'] <= 2 and token_logs and len(legs) == 1 and legs[0]['k'] == 'erc20' and int(r['v']) == 0))
        ...
        self.transfers_only = set(r['fam']) <= {'ERC20_Transfer_shape', 'Approval', 'LidoTransferShares', 'WETHDeposit', 'WETHWithdrawal', 'unknown', 'anonymous', 'DSNote'} | {f for f in r['fam'] if f.startswith('G')}
...
def render(args):
    ...
    L.append('Ethereum only, ' + (f'{hours:g} hours' if hours != 24 else 'one full day (24 hours)') + '. The deterministic step (`scripts/tx_types.py`) values successful logged native transfers, receipt-confirmed large no-log native transfers, every '
             ...
             '`scripts/type_registry.py`: each type is a qualitative description of what happens, a quantitative rule that recognises it, and an investigation method '
    ...
    L.append('Window: ' + manifest['start_utc'] + ' to ' + manifest['end_utc'] + ' (exclusive), blocks ' + f"{w['first_block']:,}" + ' to ' + f"{w['last_block']:,}" +
             ...
             f"{stats['above_threshold']:,}" + ' transactions pass the combined screen; ' + str(stats['selection_counts'].get('pc', 0)) + ' have a position change at or above ' + usd(thr) + ' (' + f"{stats['above_100k']:,}" + ' at or above $100k, ' +
        ...
        for t in ts:
            ...
            if 'pairs' in inv:
                extra.append('Pairs (sold -> bought): ' + '; '.join(x['pair'] + ' ×' + str(x['transactions']) + ' ' + usd(x['sum_usd']) for x in inv['pairs'][:8]) + '. Size median ' +
                             ...
                             '. Pools per transaction: ' + ', '.join(f'{k}: {v}' for k, v in inv['pools_per_tx'].items()) + '. Sandwiched: ' + str(inv['sandwiched']) + '.')
            if 'by_token' in inv:
    ...
    L.append(f"{cov['unknown']:,}" + ' transactions (' + usd(cov['unknown_usd']) + ') in ' + f"{cov['residue_clusters']:,}" + ' clusters keyed by destination, selector and event shape. Largest clusters:\n')
    ...
    for p in ordered:
        ...
        if is_audit(p) and not audit_started:
            audit_started = True
            L.append('## Known-type audit: one hash-sampled occurrence per type, re-read by the LLM\n')
...
def main():
    ...
    p.add_argument('--implied-min-swaps', type=int, default=20)
