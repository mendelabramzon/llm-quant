...
# REGISTRY_ROUND=1 replays the inherited registry, 2 adds the day's first batch of learned types, 3 (default) adds the second.
REGISTRY_ROUND = int(os.environ.get('REGISTRY_ROUND', '3'))
...
def trade_against_unpriced(c):
    ...
    return bool((payers & unp_in) or (receivers & unp_out))
...
DAY_TYPES = [
    T('aave_fork_lending_op', 'lending', 'lending operation on an Aave-v3-fork pool',
      ...
      lambda c: c.has(*AAVE_FAMILIES) and not c.has_at(AAVE_CORE, *AAVE_FAMILIES) and c.r['swaps'] == 0 and c.flash == 0,
      'Aave-shaped pool event from an emitter other than the core pool, no swap, no flash loan', 'lending', DAY,
    ...
    T('smart_account_execute_transfer', 'wallet', 'ERC-7821 execute() moving the account\'s own tokens',
      'An EOA calls execute(bytes32 mode, bytes executionData) on an account contract (or a delegated EOA) and the account transfers tokens it holds. The window\'s '
      'largest occurrences are USDC moves of $5M-$11M between a small set of account contracts, executed by one operator EOA: a custody stack built on smart accounts.',
...
ROUND3_TYPES = [
    ...
    T('contract_unwrap_payout', 'custody', 'WETH received by a contract and paid out as ETH',
      ...
      lambda c: c.unwrap_legs and all(l['f'] == c.r['to'] for l in c.unwrap_legs) and c.legs_into_to and all(l['s'] == 'WETH' for l in c.legs_into_to)
      and not c.legs_third_party and c.r['swaps'] == 0 and c.flash == 0,
      'WETH legs into the called contract and a WETH Withdrawal by that contract, no swap', 'custody_flow', ROUND3),
