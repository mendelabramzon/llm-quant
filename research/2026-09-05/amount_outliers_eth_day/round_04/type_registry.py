...
# REGISTRY_ROUND=1 replays the inherited registry, 2 adds the day's first batch of learned types, 3 the second, 4 (default) the audit corrections.
REGISTRY_ROUND = int(os.environ.get('REGISTRY_ROUND', '4'))
...
BRIDGE_OUT = {'OPTransactionDeposited', 'OPETHDepositInitiated', 'OPERC20DepositInitiated', 'OPETHBridgeInitiated', 'OPERC20BridgeInitiated',
              ...
              'SynapseTokenDeposit', 'SocketBridge', 'LiFiTransferStarted', 'LiFiTransferStartedObserved'}
...
TYPES = [
    ...
    T('atomic_arbitrage', 'mev', 'positive observed-net multi-pool strategy',
      ...
      lambda c: c.r['swaps'] >= 2 and len(c.r.get('pool_ids', c.r['pools'])) >= 2 and c.flash == 0 and c.pnet and all(v >= 0 for v in c.pnet.values()) and any(v > 0 for v in c.pnet.values())
      and not c.r['fnet'],
...
NEW_TYPES = [
    T('lending_with_swap', 'lending', 'lending-position adjustment with swaps',
      'Collateral or debt changes alongside a trade. A positive cash balance can be released collateral or newly borrowed inventory rather than arbitrage income; inspect supply, debt and withdrawal records separately.',
      lambda c: c.flash==0 and c.r['swaps']>0 and c.has('AaveSupply','AaveBorrow','AaveRepay','AaveWithdraw','MorphoSupply','MorphoBorrow','MorphoRepay','MorphoWithdraw'),
      'swap events plus a lending-position event, without a matched flash-loan pair', 'lending', LEARNED),
...
for entry in TYPES:
    ...
    elif entry['id'] == 'atomic_arbitrage':
        entry['what'] = 'The selected principal has a positive observed priced-asset net after multiple pool interactions. This is an arbitrage candidate; missing assets, internal ETH, claims, gas and bribes prevent a profit conclusion.'
    elif entry['id'] in ('stablecoin_mint','stablecoin_burn'):
        entry['what'] += ' This records token supply mechanics. Conversion, vault and credit operations can also mint or burn tokens; external fiat issuance or redemption is not established.'
...
# ---------------------------------------------------------------------------------------------------------------
# Types learned on the 24-hour window (2026-09-04 14:00 to 2026-09-05 14:00 UTC) by the LLM step of that run.
# They sit after every inherited type, so a specific documented mechanism always wins over these mechanical shapes.
DAY = 'day run round 2 (Claude), qual_notes.md of research/2026-09-05/amount_outliers_eth_day'
AAVE_CORE = '0x87870bca3f3fd6335c3f4ce8392d69350b4fa4e2'
AAVE_FORK_POOLS = {'0xc13e21b648a5ee794902342038ff3adab66be987': 'SparkLend pool (model memory)'}
PERMIT2 = '0x000000000022d473030f116ddee9f6b43ac78ba3'
CUSTODY_ABI = {'0x2da03409': 'sweep(address account, address token)', '0x0dcd7a6c': 'payout(address to, uint256 amount, address token, uint256 deadline, uint256 nonce)',
               '0x8568523a': 'withdraw(address token, uint256 amount) to the caller'}
OBSERVED_VENUES = {'0x1111113ccf1426a8e30e2bff5e005d929bf6a90a': 'settlement-event emitter used by resolver fills (observed; 1inch-style address)',
                   '0xb92fe925dc43a0ecde6c8b1a2709c170ec4fff4f': 'router that forwards approvals to a 0x-Settler-style contract (observed)'}
AAVE_FAMILIES = ('AaveSupply', 'AaveBorrow', 'AaveRepay', 'AaveWithdraw', 'AaveLiquidation')


def net_payers_and_receivers(c):
    payers = {who for who, _, _, _, u in c.r['net'] if Decimal(u) < 0 and who != c.ZERO}
    receivers = {who for who, _, _, _, u in c.r['net'] if Decimal(u) > 0 and who != c.ZERO}
    return payers, receivers


def unpriced_parties(c):
    """Addresses that receive or send a token the scan could not price (from the compact unpriced legs)."""
    inflow, outflow = set(), set()
    for tok, v in c.r['unp'].items():
...
def trade_against_unpriced(c):
    if c.r['swaps'] or c.flash > 0 or not c.r['unp']:
        return False
    payers, receivers = net_payers_and_receivers(c)
    unp_in, unp_out = unpriced_parties(c)
    # a receipt token minted to the payer, or burned by the receiver, is a deposit or a withdrawal, not a trade
    return bool((payers & (unp_in - c.unpriced_minted)) or (receivers & (unp_out - c.unpriced_burned)))


def fee_split(c):
    """Value enters the called contract and leaves it to two or more recipients, the contract keeps nothing, the smallest share is a fee."""
    r = c.r
    if not r['to'] or r['swaps'] or c.flash > 0 or not c.legs_into_to or len(c.legs_out_of_to) < 2:
        return False
    assets = {l['a'] for l in c.legs_into_to} | {l['a'] for l in c.legs_out_of_to}
    if len(assets) != 1:
        return False
    total_in = sum(Decimal(l['usd']) for l in c.legs_into_to)
    outs = {}
    for l in c.legs_out_of_to:
        outs[l['r']] = outs.get(l['r'], Decimal(0)) + Decimal(l['usd'])
    total_out = sum(outs.values())
    if len(outs) < 2 or total_in <= 0 or abs(total_out - total_in) > total_in / 100:
        return False
    return min(outs.values()) <= total_in / 10


def all_third_party(c):
    return bool(c.erc20_legs) and all(l['f'] not in (c.r['from'], c.r['to'], c.ZERO) and l['r'] not in (c.r['to'], c.ZERO) for l in c.erc20_legs)


DAY_TYPES = [
    T('aave_fork_lending_op', 'lending', 'lending operation on an Aave-v3-fork pool',
      'Supply, borrow, repay, withdraw or liquidation with Aave v3 event shapes emitted by a pool other than the documented Aave core pool. The window\'s '
      'occurrences are all at 0xc13e21b6…be987, which model memory identifies as the SparkLend pool; positions are USDS, cbBTC and wstETH in the $10M-$33M range.',
      lambda c: c.has(*AAVE_FAMILIES) and c.has('AaveReserveDataUpdated') and not c.has_at(AAVE_CORE, *AAVE_FAMILIES) and c.r['swaps'] == 0 and c.flash == 0,
      'Aave-shaped pool event together with ReserveDataUpdated from an emitter other than the core pool, no swap, no flash loan', 'lending', DAY,
      'The inherited spark_op rule looked for a SparkSupply family that can never match because its signature equals AaveSupply; this entry replaces it.'),
    T('trade_against_unpriced_token', 'dex', 'trade whose other side is a token outside the registry',
      'An address pays a priced asset and receives a token the scan cannot price (or the reverse) in one transaction, through a venue that emits no registered swap '
      'event: 0x-Settler-style anonymous logs, resolver fills that record per-account settlement events, OTC contracts. The trade is real but only one side is valued, '
      'so the position change overstates what the principal gave up.',
      trade_against_unpriced,
      'no registered swap event, no flash loan; some net payer of a priced asset receives an unpriced token, or a net receiver of a priced asset sends one', 'swap', DAY),
    T('permit2_operator_transfer', 'custody', 'operator moving approved tokens through Permit2',
      'A relayer calls Permit2\'s transferFrom with a batch of (owner, recipient, amount, token) entries; users\' tokens move under Permit2 allowances into one collector. '
      'The gas payer owns nothing; the asset owners approved Permit2 earlier. Seen as exchange or bridge deposit sweeps.',
      lambda c: c.r['to'] == PERMIT2 and c.r['sel'] in ('0x0d58b1db', '0x36c78516') and c.transfers_only,
      'destination is the documented Permit2 contract, selector is transferFrom (single or batch), only transfer events', 'custody_flow', DAY),
    T('smart_account_execute_transfer', 'wallet', 'ERC-7821 execute() moving the account\'s own tokens',
      'An EOA calls execute(bytes32 mode, bytes executionData) on an account and the account transfers tokens it holds. `resolve` found 23-byte EIP-7702 delegation '
      'designators on every account address in the packets (0xf90caf00…, 0x37e77355…, 0xafa1b5c1…, 0xd8eafb08…, 0x42bd41e6…, 0xc51e9af6…): these are EOAs delegated '
      'to an ERC-7821 implementation, executed by operator EOAs that pay the gas. The largest occurrences are USDC moves of $5M-$11M between the tiers of one custody stack.',
      lambda c: c.r['sel'] == '0xe9ae5c53' and c.transfers_only and c.r['swaps'] == 0 and c.legs_out_of_to and not c.legs_third_party,
      'selector execute(bytes32,bytes), only transfer events, every priced leg leaves the called account', 'custody_flow', DAY),
    T('custody_vault_abi_transfer', 'custody', 'custody vault sweep or payout by its operator ABI',
      'A vault contract of one custody system either pulls an approved balance from a client address (sweep) or pays out under a signed instruction with deadline and '
      'nonce, or hands tokens to the calling hot wallet. Four vaults share the ABI; single transfers of $15M-$28M USDC between vaults are internal rebalancing.',
      lambda c: c.r['sel'] in CUSTODY_ABI and c.transfers_only and len(c.erc20_legs) == 1 and bool(c.legs_into_to or c.legs_out_of_to),
      'one of three custody selectors (sweep, payout, withdraw), exactly one priced transfer that enters or leaves the called contract, only transfer events',
      'custody_flow', DAY),
    T('payment_router_fee_split', 'payments', 'payment routed through a contract that splits off a fee',
      'A payer\'s tokens enter a router and leave it in the same transaction to a merchant and one or more fee takers; the router keeps nothing and records the '
      'invoice in its own events. The fee is the smallest share, under a tenth of the amount.',
      fee_split, 'one asset enters the called contract and leaves it to two or more recipients within 1%; the smallest share is at most 10%', 'fee_split', DAY),
    T('relayed_payment_recorded', 'payments', 'relayed transfer of approved funds recorded by the contract',
      'An operator moves tokens from payers to recipients that are all third parties, and the called contract emits one record (often a 32-byte reference) per '
      'transfer. Neither the gas payer nor the contract holds the funds: a payment rail or settlement service executing instructions on approved balances.',
      lambda c: c.transfers_only and c.r['swaps'] == 0 and all_third_party(c) and c.r['to'] in c.custom_emitters,
      'only transfer events plus custom events from the called contract; every priced leg is third party to third party', 'custody_flow', DAY),
    T('forwarder_sweep', 'custody', 'sweep of per-customer forwarder contracts into a collector',
      'The paying address of every transfer is itself a contract that emits an event as it forwards: a deposit-forwarder pattern where each customer has a tiny contract '
      'and an operator sweeps them into a hub. The hub is the counterparty with many sources.',
      lambda c: c.transfers_only and c.erc20_legs and all(l['f'] in c.custom_emitters and l['f'] not in (c.r['from'], c.r['to']) for l in c.erc20_legs),
      'every priced leg is paid by an address that emits a custom event in the same transaction and is neither the sender nor the destination', 'custody_flow', DAY),
    T('service_deposit_with_memo', 'custody', 'deposit into a service contract with a routing memo',
      'Value enters a contract that records the deposit, and the calldata carries a printable memo such as a destination chain, token and address '
      '(for example USDT(TRON)|<address>|0.05|bridgers|): a cross-chain swap or off-chain delivery order. The contract pays out on other orders through operator calls.',
      lambda c: bool(c.r.get('memo')) and c.transfers_only and c.r['swaps'] == 0 and (c.legs_into_to or (int(c.r['v']) > 0 and not c.erc20_legs and c.r['lc'] >= 1))
      and not c.legs_out_of_to and not c.legs_third_party,
      'calldata holds a printable memo of 8+ characters; value or tokens enter the called contract; the contract emits its own event', 'custody_flow', DAY),
    T('contract_deposit_recorded', 'custody', 'deposit into a contract that records it',
      'Native value or tokens enter the called contract, nothing leaves, and the contract (or a registry it calls) emits its own event: custody hubs, bridge-like '
      'escrows, staking or trading-platform treasuries. What the deposit buys is not visible on Ethereum.',
      lambda c: c.transfers_only and c.r['swaps'] == 0 and bool(c.custom_emitters) and not c.legs_out_of_to and not c.legs_third_party
      and (c.legs_into_to or (int(c.r['v']) > 0 and not c.erc20_legs and c.r['lc'] >= 1)),
      'value or tokens enter the called contract only, a custom event is emitted, no swap', 'custody_flow', DAY),
    T('custody_withdrawal_to_operator', 'custody', 'contract pays its own tokens to the calling address',
      'The called contract sends tokens it holds to the transaction sender: a hot wallet replenishing itself from a vault, or an owner draining a contract.',
      lambda c: c.transfers_only and c.r['swaps'] == 0 and c.legs_out_of_to and not c.legs_into_to and not c.legs_third_party
      and all(l['r'] == c.r['from'] for l in c.legs_out_of_to),
      'every priced leg goes from the called contract to the transaction sender, only transfer events', 'custody_flow', DAY),
    T('contract_payout_by_operator', 'custody', 'contract-held tokens paid out by an operator call',
      'An operator EOA calls a contract that pays tokens it holds to other addresses, usually with a per-payment record: withdrawal processors, treasury payouts, '
      'settlement of cross-chain orders. The recipient profile says whether the money reaches a fresh address (customer) or another hub.',
      lambda c: c.transfers_only and c.r['swaps'] == 0 and c.legs_out_of_to and not c.legs_into_to and not c.legs_third_party,
      'every priced leg leaves the called contract to addresses other than the sender, only transfer events', 'custody_flow', DAY),
    T('operator_transfer_recorded', 'custody', 'operator-mediated transfer with custom events',
      'Tokens move between third parties, or into and out of a contract, in a transfer-only transaction that also emits custom events the registry does not name. '
      'The residual mechanical shape of custody and settlement infrastructure.',
      lambda c: c.transfers_only and c.r['swaps'] == 0 and c.erc20_legs and bool(c.custom_emitters),
      'only transfer events plus custom events; priced ERC-20 legs present; no rule above matched', 'custody_flow', DAY),
]
SUSDE = '0x9d39a5de30e57443bff2a8307a4256c8797a3497'
ROUND3 = 'day run round 3 (Claude), qual_notes.md of research/2026-09-05/amount_outliers_eth_day'
ROUND3_TYPES = [
    T('susde_unstake_claim', 'vault', 'sUSDe cooldown claim (unstake)',
      'After the cooldown, unstake(receiver) on the sUSDe contract releases USDe from the silo to the receiver; the shares were burned when the cooldown started, '
      'so this transaction shows only the USDe leaving the silo.',
      lambda c: c.r['to'] == SUSDE and c.r['sel'] == '0xf2888dbb' and any(l['s'] == 'USDe' for l in c.erc20_legs),
      'destination is the sUSDe contract, selector unstake(address) 0xf2888dbb, a USDe leg is present', 'generic', ROUND3),
    T('delegated_account_self_call', 'wallet', 'delegated EOA executing a batch on itself',
      'The transaction sender and destination are the same address and the call emits logs, which is only possible when the EOA carries an EIP-7702 delegation: '
      'the account executes a batch of transfers or settlements from its own code. Seen settling market-making inventory between several sub-accounts.',
      lambda c: c.r['to'] == c.r['from'] and c.r['lc'] >= 1,
      'destination equals sender and at least one log', 'custody_flow', ROUND3),
    T('bilateral_exchange_settled', 'dex', 'two-party exchange through a settlement contract without a swap event',
      'One party pays asset X and receives asset Y while a counterparty does the reverse, both through a contract that records the order (RFQ fill, OTC settlement, '
      'limit-order match). No pool is involved, so no registered swap event fires; the ETH side may be delivered by unwrapping, which is invisible in receipts.',
      lambda c: c.r['swaps'] == 0 and c.flash == 0 and bool(c.two_sided_parties) and len(c.assets) >= 2,
      'no swap event, no flash loan, some address other than the destination nets negative in one priced asset and positive in another, two or more priced assets',
      'swap', ROUND3),
    T('staking_pool_stake', 'staking', 'stake into a StakingRewards-style contract',
      'Tokens enter the called contract and it emits Staked(address,uint256): a farm or rewards contract records the position; rewards accrue off the ledger.',
      lambda c: c.has('Staked') and c.legs_into_to and not c.legs_out_of_to and c.r['swaps'] == 0,
      'Staked event and tokens entering the called contract, no swap', 'generic', ROUND3),
    T('staking_pool_withdraw', 'staking', 'withdrawal from a StakingRewards-style contract',
      'The called contract emits Withdrawn(address,uint256) and returns tokens to the staker.',
      lambda c: c.has('Withdrawn') and c.legs_out_of_to and c.r['swaps'] == 0,
      'Withdrawn event and tokens leaving the called contract, no swap', 'generic', ROUND3),
    T('contract_unwrap_payout', 'custody', 'WETH received by a contract and paid out as ETH',
      'WETH enters the called contract and the contract unwraps it in the same transaction; the ETH then leaves by an internal transfer that receipts do not show. '
      'The visible payer is often a liquidity wallet filling a cross-chain or off-chain order; the beneficiary is only visible in the contract\'s own event.',
      lambda c: c.unwrap_legs and all(l['f'] == c.r['to'] or l['f'] in c.custom_emitters for l in c.unwrap_legs) and all(l['s'] == 'WETH' for l in c.legs_into_to)
      and not c.legs_third_party and not c.legs_out_of_to and c.r['swaps'] == 0 and c.flash == 0,
      'a WETH Withdrawal by the called contract or by a contract that emits its own event, at most WETH legs into the called contract, no swap', 'custody_flow', ROUND3),
    T('native_deposit_wrapped_forwarded', 'vault', 'ETH deposited, wrapped and forwarded by a router',
      'Native value enters the called contract, which wraps it and passes the WETH on to a vault or strategy that records shares or totals in its own events. '
      'A deposit into a yield product whose events the registry does not name.',
      lambda c: int(c.r['v']) > 0 and c.wrap_legs and all(l['r'] == c.r['to'] for l in c.wrap_legs) and c.legs_out_of_to
      and all(l['s'] == 'WETH' for l in c.legs_out_of_to) and c.r['swaps'] == 0 and c.flash == 0,
      'top-level value, a WETH Deposit credited to the called contract and WETH legs leaving it, no swap', 'custody_flow', ROUND3),
    T('operator_sweep_no_events', 'custody', 'batched transferFrom sweep into one collector',
      'An operator pulls approved balances from one or several customer addresses into a single collector in one call; nothing else happens and no event beyond the '
      'transfers is emitted. Exchange or payment-service deposit consolidation.',
      lambda c: c.transfers_only and c.r['swaps'] == 0 and all_third_party(c) and not c.custom_emitters and len({l['r'] for l in c.erc20_legs}) == 1,
      'only transfer events, every priced leg is third party to a single recipient, no custom event', 'custody_flow', ROUND3),
    T('sender_tokens_via_contract', 'transfer', 'sender\'s own tokens sent through a contract call',
      'The transaction sender\'s tokens move to fewer than five recipients through a helper contract (multisend, app-specific transfer) that emits nothing else.',
      lambda c: c.transfers_only and c.r['swaps'] == 0 and c.erc20_legs and all(l['f'] == c.r['from'] and l['r'] not in (c.r['to'], c.ZERO) for l in c.erc20_legs),
      'only transfer events, every priced leg is paid by the sender to addresses other than the destination', 'custody_flow', ROUND3),
]
ROUND4 = 'day run round 4 (Claude): audit corrections and long-tail shapes, qual_notes.md of research/2026-09-05/amount_outliers_eth_day'
AGG_SWAP = {'AggregatorSwapped', 'GExchange_aua', 'LiFiSwapObserved', 'ZeroExTransformedERC20', 'GSwapped_abuu', 'GSold_auu', 'GBought_auuu', 'GBought_auu', 'DODOSwap'}
CLAIMS = {'Claimed', 'ClaimedIAU', 'GClaimed_aau', 'GClaimed_aaau', 'GClaimed_uauu', 'GClaim_bb', 'RewardPaid'}
ROUND4_FRONT = [
    T('dex_swap_aggregator_event', 'dex', 'swap recorded by an aggregator or venue event',
      'An aggregator router (Kyber-style Swapped/Exchange, LiFi generic swap, 0x TransformedERC20, DODO) records the trade in its own event while the pools it used '
      'may emit no registered swap event. The audit found such transactions typed as bilateral exchanges; this entry precedes that rule.',
      lambda c: c.has(*AGG_SWAP) and c.flash == 0,
      'an aggregator swap event family is present, no flash loan', 'swap', ROUND4),
    T('unregistered_token_mint_burn', 'stablecoin', 'issuance or redemption of a token priced from its own swaps',
      'Supply of a token outside the registry is created or destroyed for at least the threshold, valued at its in-sample price; the window\'s cases are dollar-pegged '
      'tokens minted or burned by their issuer contracts.',
      lambda c: any(k.startswith('IMPL:') and (Decimal(a) >= c.thr or Decimal(b) >= c.thr) for k, (a, b) in c.r['mb'].items()) and c.r['swaps'] == 0 and c.flash == 0,
      'an implied-priced token minted from or burned to the zero address for at least the threshold, no swap', 'mint_burn', ROUND4),
    T('receipt_token_minted_deposit', 'vault', 'deposit that mints an unpriced receipt token',
      'A priced asset leaves the principal and a token the registry cannot price is minted in the same transaction: shares of a vault, fund or pool whose events the '
      'registry does not name. The audit found these typed as trades; a minted receipt is a claim, not a counterparty payment.',
      lambda c: c.r['swaps'] == 0 and c.flash == 0 and bool(c.unpriced_minted) and bool(net_payers_and_receivers(c)[0]),
      'no swap event, an unpriced token minted from the zero address, some address nets negative in a priced asset', 'custody_flow', ROUND4),
    T('receipt_token_burned_withdrawal', 'vault', 'withdrawal that burns an unpriced receipt token',
      'The reverse: an unpriced token is burned and a priced asset reaches the principal.',
      lambda c: c.r['swaps'] == 0 and c.flash == 0 and bool(c.unpriced_burned) and bool(net_payers_and_receivers(c)[1]),
      'no swap event, an unpriced token burned to the zero address, some address nets positive in a priced asset', 'custody_flow', ROUND4),
    T('deposit_with_nft_receipt', 'vault', 'position or order opened with an NFT receipt',
      'A priced asset leaves the sender and an ERC-721 is minted to the sender: a position, order or vault share represented as an NFT (the window\'s case routes WBTC '
      'into a 0x-Settler-style contract and mints the receipt).',
      lambda c: c.r['rc721'] >= 1 and c.r['swaps'] == 0 and c.flash == 0 and any(l['f'] == c.r['from'] for l in c.erc20_legs) and not c.has('SeaportOrderFulfilled', 'BlurOrdersMatched'),
      'an ERC-721 transfer, a priced leg paid by the sender, no swap, no marketplace event', 'custody_flow', ROUND4),
    T('claim_distribution', 'transfer', 'claim of vested, distributed or reward tokens',
      'The called contract pays tokens it holds and emits a Claimed-style event: vesting, airdrop or reward distribution.',
      lambda c: c.has(*CLAIMS) and c.r['swaps'] == 0 and c.flash == 0 and (c.legs_out_of_to or c.r['unp']),
      'a Claimed-style event family with tokens leaving the called contract, no swap', 'custody_flow', ROUND4),
    T('relay_style_deposit_observed', 'custody', 'deposit recorded with a Relay-style deposit event',
      'A depository other than the documented Relay contract emits the same RelayNativeDeposit or RelayErc20Deposit shape as value enters it; the inherited relay_deposit '
      'rule pins one address, this entry records the shape wherever it appears.',
      lambda c: c.has('RelayNativeDeposit', 'RelayErc20Deposit') and c.r['swaps'] == 0 and c.flash == 0 and (c.legs_into_to or int(c.r['v']) > 0),
      'Relay-style deposit event from any emitter with value or tokens entering the called contract', 'custody_flow', ROUND4),
    T('aster_style_withdrawal_observed', 'trading_custody', 'withdrawal recorded with the observed Aster withdrawal topic',
      'Strategy or treasury contracts other than the documented Aster treasury emit the same withdrawal topic as tokens leave them to a user: the inherited rule pins one '
      'address, this entry records the shape.',
      lambda c: c.has('AsterWithdrawalObserved') and c.r['swaps'] == 0 and c.flash == 0 and bool(c.erc20_legs),
      'the observed Aster withdrawal topic from any emitter with priced token legs', 'custody_flow', ROUND4),
]
ROUND4_TAIL = [
    T('native_call_into_contract_silent', 'custody', 'ETH sent with calldata into a contract that emits nothing',
      'A confirmed-successful call carrying ETH into a contract that is called by many and emits no log: a deposit contract without events (exchange, bridge or staking '
      'router). The window\'s cases are bots paying 13-52 ETH per call to one contract, dozens of times.',
      lambda c: int(c.r['v']) > 0 and c.r['cd'] > 0 and c.r['lc'] == 0 and bool({'contract_like', 'router_like'} & c.to_tags),
      'top-level value, calldata, no logs, destination tagged contract_like or router_like', 'custody_flow', ROUND4),
    T('contract_deposit_no_event', 'custody', 'deposit into a contract without a record event',
      'Tokens enter the called contract and nothing else happens: vault top-ups by hot wallets, converter deposits, escrow funding.',
      lambda c: c.transfers_only and c.r['swaps'] == 0 and c.legs_into_to and not c.legs_out_of_to and not c.legs_third_party and not c.custom_emitters,
      'only transfer events, every priced leg enters the called contract, no custom event', 'custody_flow', ROUND4),
]
if REGISTRY_ROUND >= 4:
    _after = [t['id'] for t in DAY_TYPES].index('aave_fork_lending_op') + 1  # lending before receipt-token shapes: aTokens are minted receipts
    DAY_TYPES[_after:_after] = ROUND4_FRONT
    _idx = [t['id'] for t in DAY_TYPES].index('contract_deposit_recorded')
    DAY_TYPES[_idx:_idx] = ROUND4_TAIL
if REGISTRY_ROUND >= 3:
    _idx = [t['id'] for t in DAY_TYPES].index('contract_deposit_recorded')
    DAY_TYPES[_idx:_idx] = ROUND3_TYPES
if REGISTRY_ROUND >= 2:
    TYPES.extend(DAY_TYPES)
    for entry in TYPES:
        if entry['id'] == 'spark_op':
            entry['rule'] = lambda c: False
            entry['rule_text'] = 'retired on the day run: its SparkSupply family shares the AaveSupply signature and can never match; see aave_fork_lending_op'
